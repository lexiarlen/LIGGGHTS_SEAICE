...
def plot_stress_strain(ds_stress, ds_strain, output_directory):
    fig = plt.figure()
    plt.plot(ds_strain['strain'].values, ds_stress['axial_stress'].values, color = 'maroon')
    plt.ylabel('Axial Stress [Pa]')
    plt.xlabel('Axial Strain [-]')
    plt.grid()
    outpath = os.path.join(output_directory, 'stress_strain.jpg')
    plt.savefig(outpath, dpi = 300)
    plt.close()
...
if __name__ == '__main__':
    # Interactive inputs
    rel_path = input(
        "Enter base path (from '/mnt/c/Users/arlenlex/Documents/liggghts_data/ucs/simulations'): "
    )
    base_path = os.path.join('/mnt/c/Users/arlenlex/Documents/liggghts_data/ucs/simulations', rel_path)

    dt_input = input("Enter dt (default: 0.00005): ")
    dt = float(dt_input) if dt_input else 0.00005
    ...
    if stress_strain:
        fpath_stress = os.path.join(base_path, 'stress.nc')
