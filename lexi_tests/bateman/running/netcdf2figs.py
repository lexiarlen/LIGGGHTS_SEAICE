...
def process_bond_file(ds_b, num_atoms):
    ...
    return n_bonds, adjacency_matrix, coordination_numbers_df # average_bond_force_array,
...
def get_compresive_stress_in_volume(ds_a, ds_b):
    '''
    uses the avg stress equation to get the avg compressive stress
    sigma_yy in the volume. need to add to the parse bond function. 
    '''
    fac = get_bond_multiplier_factor(ds_a)
    sig_yys = []
    # parse bond dataset
    group_names = list(ds_b.groups.keys()) # these are timesteps
    for idx in range(len(group_names)):
        group_name = group_names[idx]
        group = ds_b.groups[group_name]
        # get y data
        batom1y = group.variables['batom1y'][:]
        batom2y = group.variables['batom2y'][:]
        bforcey = group.variables['bforceY'][:]
        # compute compressive stress
        sig_yy = (0.5*abs(batom1y - batom2y)*bforcey).sum()
        sig_yys.append(sig_yy)
    contact_stresses = ds_a['c_stress[2]'].mean(dim='id').values # are these computed w the formula
    return fac*np.array(sig_yys) + contact_stresses

def create_atom_positions_animation(ds, output_directory, dt, coordnums_df, frame_skip_value=5):
    '''
    in:
    ...
    y0 = ds['z'].isel(timestep=0).values  # Shape: (id,)
    ...
    def update(frame):
        ...
        y = ds['z'].isel(timestep=frame).values
        ...
        timestep_value = np.round(ds['timestep'].values[frame]*dt, decimals = 5)
    ...
    ani.save(fpath, writer='Pillow', fps=10)
...
if __name__ == '__main__':
    ...
    time_average_data_input = input("Time average data? (Y/N, default: N): ")
    time_average_data = time_average_data_input.lower() == 'y' if time_average_data_input else False

    coordnum_gif_input = input("Create coordination number GIF? (Y/N, default: N): ")
    coordnum_gif = coordnum_gif_input.lower() == 'y' if coordnum_gif_input else False

    coordnum_initial_input = input("Plot the initial coordination numbers? (Y/N, default: N): ")
    ...
    fsd_input = input("Compute FSD? (Y/N, default: N): ")
    ...
    # Open datasets
    fpath_a = os.path.join(base_path, 'all_atoms.nc')
    ...
    if final_floes or time_average_data or coordnum_gif or fsd or coordnum_initial:
    ...
    if coordnum_gif:
        create_atom_positions_animation(ds_a, output_directory, dt, coordination_numbers_df, frame_skip_value=5)
    ...
    if time_average_data:
        ...
        time_avg_quantities = [ avg_fmag, avg_vmag, nbonds, bond_force, ex_ydisplacement, avg_co]
        units = [ '[N]', '[m/s]', '[#]', '[N]', '[m]', '[#]']
        titles = [ r'$|\overline{\bf{F}_a}|$', r'$|\overline{\bf{v}}|$',
                  '# bonds', r'$|\overline{\bf{F}_b}|$', r'Atom 100 y displacement', r'$\bar{Co}$ #']
        output_names = ['a_fmag.jpg', 'vmag.jpg', 'nbonds.jpg', 'b_fmag.jpg', 'ydisp.jpg', 'avg_coordnum.jpg']
