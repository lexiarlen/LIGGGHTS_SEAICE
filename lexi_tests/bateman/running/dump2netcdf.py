...
def main():
    ...
    parser.add_argument('--column_names', default=None, help='Column names for bond data (default is "batom1 batom2 bbondbroken")')
    ...
    sample_files = sorted(glob.glob(os.path.join(post_dir, 'sample*.liggghts')))
    top_plate_files = sorted(glob.glob(os.path.join(post_dir, 'top_plate*.liggghts')))
    all_atoms_files = sorted(glob.glob(os.path.join(post_dir, 'dump*.liggghts')))
    bond_files = sorted(glob.glob(os.path.join(post_dir, 'bfc*.bond')))

    if not sample_files:
        print(f"Error: No atom dump files (sample*.liggghts) found in the post directory '{post_dir}'.")
        return
    
    if not top_plate_files:
        print(f"Error: No atom dump files (top_plate*.liggghts) found in the post directory '{post_dir}'.")
        return
    
    if not all_atoms_files:
    ...
    # # Process sample data
    sample_output_path = os.path.join(out_dir, 'atoms_sample.nc')
    if os.path.isfile(sample_output_path):
        os.remove(sample_output_path)
    get_atom_ds(post_dir, 'sample*.liggghts', sample_output_path)
    ...
    # Process all atoms data
    atoms_output_path = os.path.join(out_dir, 'all_atoms.nc')
    if os.path.isfile(atoms_output_path):
        os.remove(atoms_output_path)
    get_atom_ds(post_dir, 'dump*.liggghts', atoms_output_path)
