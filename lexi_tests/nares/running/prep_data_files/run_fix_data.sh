...
base_path="/home/arlenlex/LIGGGHTS_SEAICE/lexi_tests/nares/simulations/create_packing_better/data"
...
if [[ -z "$bond_skin_thickness" ]]; then
    ...
    python fix_data5.py "$xlo" "$xhi" "$ylo" "$yhi" "$repo"
else
    # Call Python script with bond_skin_thickness
    python fix_data5.py "$xlo" "$xhi" "$ylo" "$yhi" "$repo" "$bond_skin_thickness"
fi
