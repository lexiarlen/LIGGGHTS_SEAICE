...
# Workflow steps
base_path="/home/arlenlex/LIGGGHTS_SEAICE/lexi_tests/ji/simulations/nomesh" 
output_path="/mnt/c/Users/arlenlex/Documents/liggghts_data/ji/simulations/nomesh"
experiment_name="damping"
processors_install=1  
processors_load=3

for packing in ji_nomesh1 ji_nomesh2; do 
    for damping in 0.001 0.1; do
        echo "RUNNING PACKING: $packing with damping factor $damping"

        # Step 1: Create output directory
        output_dir=$(create_output_dirs "$output_path" "$experiment_name" "${packing}_damp${damping}")
        ...
        # Step 3: Run in.bond
        sed "s|read_data .*|read_data data/${packing}.data|; s|variable bond_damp_val .*|variable bond_damp_val equal ${damping}|; \
             s|write_restart .*|write_restart restarts/${packing}.restart|" \
        ...
        sed "s|read_restart .*|read_restart restarts/${packing}.restart|; s|variable post_dir .*|variable post_dir string "post_${experiment_name}_${packing}"|; \
            s|variable bond_damp_val .*|variable bond_damp_val equal ${damping}|" \
        ...
        if [ ! -f "$base_path/post_${experiment_name}_${packing}" ]; then
            ...
            python3 nc2figs.py --output-dir "$output_dir" --dt 0.00001
        else 
