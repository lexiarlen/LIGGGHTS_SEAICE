...
def main():
    """
    Usage: python fix_data.py xlo xhi ylo yhi zlo zhi repo bond_skin_thickness
    """
    if len(sys.argv) == 9:
        _, xlo, xhi, ylo, yhi, zlo, zhi, original_fpath, bond_skin_thickness = sys.argv
        bond_skin_thickness = float(bond_skin_thickness)
    elif len(sys.argv) == 8:
        _, xlo, xhi, ylo, yhi, zlo, zhi, original_fpath = sys.argv
        bond_skin_thickness = 0.005*0.015
    else:
        print("Usage: python fix_data.py xlo xhi ylo yhi zlo zhi repo bond_skin_thickness")
        sys.exit(1)
