...
for norm in 1e6 5e6 10e6 50e6; do
    for shear in 5e6 10e6 50e6; do
        ...
        norm_bc=$(convert_for_bc "$norm")
        shear_bc=$(convert_for_bc "$shear")

        check=$(echo "$shear_bc >= $norm_bc" | bc -l)
        if [ "$check" -eq 0 ]; then
            continue
        fi

        # Step 1: Create output directory
        variant="norm_${norm}_shear_${shear}"
        ...
        sed "s|read_restart .*|read_restart restarts/$variant.restart|; s|variable normal_strength .*|variable normal_strength equal $norm|; \
            s|variable shear_strength .*|variable shear_strength equal $shear|; s|variable post_dir .*|variable post_dir string "post_${experiment_name}_${variant}"|;" \
